...
VERSION = "hbm2_openai_colab_v2_concurrent"
...
async def process_one_sample(
    *,
    ...
    """Run CALL 1 and CALL 2 concurrently, then run dependent CALL 3."""
    ...
    try:
        # CALL 1 and CALL 2 are independent, so issue them at the same time.
        ...
        decision_obj, decision_raw, decision_usage, decision_request_id = (
            await structured_response_call(
                client,
                semaphore,
                model=args.model,
        ...
        return {
            ...
            "memory_update_mode": "batch_boundary",
            "prompts": {
...
async def async_main() -> None:
    ...
    if args.concurrent_samples < 1:
        raise ValueError("--concurrent_samples must be at least 1")
    ...
    print("=" * 78)
    print("FLARE-VAX HBM2 — Concurrent OpenAI API / Colab Validation")
    print("=" * 78)
    ...
    print(f"Concurrent samples      : {args.concurrent_samples}")
    ...
    if args.dry_run:
        ...
        print(THREAT_USER_TEMPLATE.format(profile=build_threat_profile(first)))
        print("\n--- DRY RUN: CALL 2 INPUT ---")
        print(BARRIER_USER_TEMPLATE.format(profile=build_barrier_profile(first)))
        print("\n--- DRY RUN: CALL 3 CONTEXT SKELETON ---")
        print(build_context_profile(first, args.include_sensitive_context))
    ...
    memory_added_indices: set[int] = set()
    for idx in range(min(start_idx, train_size)):
        ...
        if entry and entry.get("status") == "ok" and not bool(entry.get("is_correct")):
            ...
            memory_added_indices.add(idx)

    print(f"Resume position          : {start_idx}/{total_size}")
    print(f"Rebuilt memory           : {memory.size()} training errors")
    print(
        "Concurrency note        : memory is updated after each batch; respondents "
        "inside the same batch use the same pre-batch memory snapshot."
    )
    ...
    for batch_start in tqdm(
        range(start_idx, total_size, args.concurrent_samples),
        desc="FLARE-VAX concurrent batches",
    ):
        block_indices = list(
            range(batch_start, min(batch_start + args.concurrent_samples, total_size))
        )
        # A previous concurrent run may have completed later entries in a block even
        # when an earlier entry failed. Skip already-successful entries on resume.
        batch_indices = [
            idx
            for idx in block_indices
            if latest.get(idx, {}).get("status") != "ok"
        ]
        tasks = []
        ...
        batch_entries = await asyncio.gather(*tasks) if tasks else []
        batch_entries = sorted(batch_entries, key=lambda entry: int(entry["data_idx"]))

        # Log in deterministic index order. Memory is updated only after all requests
        # in this batch have completed, preventing race-dependent memory contents.
        for entry in batch_entries:
            ...
            if entry.get("status") == "ok":
                ...
                idx = int(entry["data_idx"])

                if newly_processed <= args.print_samples:
                    print("\n" + "=" * 78)
                    print(
                        f"SAMPLE {idx} | phase={entry['phase']} | actual={entry['actual']} | "
                        f"decision={entry['decision']} | correct={entry['is_correct']}"
                    )
                    print(
                        f"Pattern: P{entry['hbm2_pattern']} {entry['pattern_label']}"
                    )
                    print(
                        "Threat output: "
                        + json.dumps(entry["threat_result"], ensure_ascii=False)
                    )
                    print(
                        "Barrier output: "
                        + json.dumps(entry["barrier_result"], ensure_ascii=False)
                    )
                    print(f"Decision output: {entry['raw_outputs']['decision']}")
                    print(f"Memory examples used: {entry['memory_source_indices']}")
                    print(f"Tokens: {entry['usage_total']}")
                    print("=" * 78)
            else:
                print(
                    f"\nERROR at sample {entry['data_idx']}: "
                    f"{entry['error_type']}: {entry.get('error_message', '')}",
                    file=sys.stderr,
                )
                if not args.continue_on_error:
                    stop_requested = True

        # Advance memory only at the batch boundary and in original data order.
        # This also incorporates already-completed entries that were skipped on resume.
        for idx in block_indices:
            if idx >= train_size or idx in memory_added_indices:
                continue
            completed_entry = latest.get(idx)
            if (
                completed_entry
                and completed_entry.get("status") == "ok"
                and not bool(completed_entry.get("is_correct"))
            ):
                memory.store(
                    idx, feature_matrix[idx], build_memory_record(completed_entry)
                )
                memory_added_indices.add(idx)

        if (
            args.checkpoint_every > 0
            and processed_since_checkpoint >= args.checkpoint_every
        ) or stop_requested:
        ...
        if stop_requested:
            print(
                "Stopping after the current concurrent batch because at least one "
                "sample failed. Re-run the same command to retry from the first "
                "unfinished sample, or add --continue_on_error.",
                file=sys.stderr,
            )
            break
